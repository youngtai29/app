        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="goodAttrs in spuSaleAttrList" :key="goodAttrs.id">
                <dt class="title">
                ...
                <dd
                  ...
                  "
                  :class="{
                    active: saleName.isChecked !== '0',
                  }"
                  changeprice="0"
                  v-for="(saleName, index) in goodAttrs.spuSaleAttrValueList"
                  changepirce="40"
                >
                  {{ saleName.saleAttrValueName }}
                </dd>
              </dl>
            </div>
            <div class="cartWrap">
              <div class="controls">
                <!-- number可以将输入的数据转换为Number类型,但如果不能转换，会按原值保存（此法失效） -->
                <!-- 京东商城在输入（oninput）的时候就校验，有一定的时间间隔（1s） -->
                <input
                  @change="changeGoodsNum"
                  @keyup.enter="inputBlur"
                  autocomplete="off"
                  class="itxt"
                  v-model.number.lazy="goodsNum"
                />
                <a href="javascript:" class="plus" @click="addNum">+</a>
                <a
                  href="javascript:"
                  :class="{ 'no-select': goodsNum <= 1 }"
                  class="mins"
                  @click="decNum"
                  >-</a
                >
...
export default {
  ...
  computed: {
    // goodsNum(){
    //   if(this.goodsNum&&this.goodsNum.trim()==)
    ...
    spuSaleAttrList() {
      // ATN[]!==[]//true
      // if (
      //   this.goodsInfo.spuSaleAttrList &&
      //   this.goodsInfo.spuSaleAttrList.length === 0
      // ) {
      //   console.log("xx");

      //   return this.skuSaleAttrList;
      // } else
      return this.goodsInfo.spuSaleAttrList || [{}];
    },
    skuSaleAttrList() {
      return this.goodsInfo.skuInfo.skuSaleAttrValueList || [{}];
    },
  },
  methods: {
    changeActive(saleName, list) {
      list.forEach((item) => {
        item.isChecked = "0";
      });
      saleName.isChecked = "1";
    },
    addNum() {
      this.goodsNum += 1;
    },
